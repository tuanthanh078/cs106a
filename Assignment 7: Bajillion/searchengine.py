...
---------------------
This program implements a simple text-based search engine. It's like Google, only
smaller. It consists of two main parts. The first one is updating the index
and titles dictionary from a list of file names. The second one is the actual search
functionality.
"""
...
def create_index(filenames, index, file_titles):
    ...
    for filename in filenames:
        with open(filename) as file:
            ...
            process_line(index, filename, first_line)
            file_titles[filename] = first_line.strip()
            for line in file:
                process_line(index, filename, line)

def process_line(index, filename, line):
    """
    This function is passed:
        index:          a dictionary mapping from terms to file names (i.e., inverted index)
                        (term -> list of file names that contain that term)

        filename:       a name of a file (string)

        line:           a line in the file called filename

    The function will update the index passed in to include the terms in the line.
    """
